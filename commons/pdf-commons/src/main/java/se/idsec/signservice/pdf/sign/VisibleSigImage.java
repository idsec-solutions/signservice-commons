...
public abstract class VisibleSigImage {
   ...
   */
  public VisibleSigImage(int page, int xOffset, int yOffset, int zoomPercent, Map<String, String> personalizationParams) {
    this.page = page;
  ...
  /**
   * Provide the location of the SVG image source, specified as absolute file path or as classpath. Location specified as classpath
   * is specified as relative path preceded with "classpath:". E.g "classpath:img/SC-sign-image.svg"
   * @return Width in pixels
   */
  ...
  private String getSvgImage() throws IOException {
    String imageLocation = getImageLocation();
    InputStream is;
    if (imageLocation.startsWith("classpath:")){
      is = getClass().getResourceAsStream("/" + imageLocation.substring(10));
    } else {
      is = new FileInputStream(new File(imageLocation));
    }
    String svg = new String(IOUtils.toByteArray(is));
    return svg;
  }
