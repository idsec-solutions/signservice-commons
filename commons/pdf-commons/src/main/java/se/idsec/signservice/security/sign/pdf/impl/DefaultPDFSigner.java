...
public class DefaultPDFSigner implements PDFSigner {
   ...
   * @throws NoSuchAlgorithmException if the supplied signature algorithm is not supported
   */
  public DefaultPDFSigner(final PkiCredential signingCredential, final String signatureAlgorithm)
  ...
  public PDFSignerResult sign(final byte[] document, final PDFSignerParameters parameters) throws SignatureException {
    ...
    try (final PDDocument pdfDocument = Loader.loadPDF(document)) {
      final List<X509Certificate> signingCertChain = this.includeCertificateChain
          ...
          : Collections.singletonList(this.signingCredential.getCertificate());
      ...
      final PDFSigningProcessor.Result signatureResult =
          PDFSigningProcessor.signPdfDocument(pdfDocument, signatureProvider, signingTime,
